...
public class LeaveService {
    ...
    @Transactional
    public LeaveDTOs.Response applyLeave(Long employeeId, LeaveDTOs.CreateRequest req) {
        ...
        if (!leaveBalanceService.hasSufficientBalance(emp, req.getLeaveType(), days)) {
            throw new IllegalStateException(
                    "Insufficient leave balance for " + req.getLeaveType() + ". Requested: " + days + " days");
        }

        Employee manager = req.getManagerId() != null
                ? employeeService.findById(req.getManagerId()) : null;
        ...
        LeaveRequest leave = LeaveRequest.builder()
                ...
                .approvalStage(manager != null ? ApprovalStage.MANAGER_PENDING : ApprovalStage.HR_PENDING)
                .build();

        LeaveRequest saved = leaveRepo.save(leave);

        Employee notifyTarget = manager != null ? manager : emp;
        notificationService.createAndSend(
                notifyTarget,
                "New Leave Request",
                emp.getFirstName() + " " + emp.getLastName() + " applied for " + days + " day(s) of " + req.getLeaveType() + " leave.",
    ...
    @Transactional
    public LeaveDTOs.Response managerAction(Long leaveId, Long managerId, LeaveDTOs.ActionRequest req) {
        LeaveRequest leave = findById(leaveId);

        if (leave.getApprovalStage() != ApprovalStage.MANAGER_PENDING) {
            throw new IllegalStateException("Leave is not awaiting manager approval");
        }
        ...
        if (req.getAction() == LeaveStatus.APPROVED) {
            ...
            notificationService.createAndSend(
                    ...
                    "Your leave request was approved by manager and is now pending HR verification.",
                    NotificationType.LEAVE_APPLIED, "LEAVE_REQUEST", leave.getId());
        } else {
            ...
            notificationService.createAndSend(
                    leave.getEmployee(),
                    "Leave Rejected",
                    "Your leave request was rejected by your manager. Reason: " + req.getRemarks(),
                    NotificationType.LEAVE_REJECTED, "LEAVE_REQUEST", leave.getId());
        }
    ...
    @Transactional
    public LeaveDTOs.Response hrAction(Long leaveId, Long hrId, LeaveDTOs.ActionRequest req) {
        LeaveRequest leave = findById(leaveId);

        if (leave.getApprovalStage() != ApprovalStage.HR_PENDING) {
            throw new IllegalStateException("Leave is not awaiting HR verification");
        }
        ...
        if (req.getAction() == LeaveStatus.APPROVED) {
            ...
            leaveBalanceService.deductBalance(leave.getEmployee(), leave.getLeaveType(), leave.getTotalDays());

            notificationService.createAndSend(
                    leave.getEmployee(),
                    "Leave Approved",
                    "Your " + leave.getLeaveType() + " leave from " + leave.getStartDate() + " to " + leave.getEndDate() + " has been approved.",
                    NotificationType.LEAVE_APPROVED, "LEAVE_REQUEST", leave.getId());
        } else {
            leave.setApprovalStage(ApprovalStage.REJECTED);
            leave.setStatus(LeaveStatus.REJECTED);
            ...
            notificationService.createAndSend(
                    leave.getEmployee(),
                    "Leave Rejected",
                    "Your leave request was rejected by HR. Reason: " + req.getRemarks(),
                    NotificationType.LEAVE_REJECTED, "LEAVE_REQUEST", leave.getId());
        }
    ...
    @Transactional
    public LeaveDTOs.Response requestCancellation(Long leaveId, Long employeeId, LeaveDTOs.CancelRequest req) {
        LeaveRequest leave = findById(leaveId);
        ...
        if (leave.getStatus() == LeaveStatus.APPROVED) {
            // Already approved — needs HR confirmation before reversing
            ...
            notificationService.createAndSend(
                    ...
                    "Your cancellation request for " + leave.getLeaveType() + " leave is pending HR confirmation.",
                    NotificationType.LEAVE_CANCELLED, "LEAVE_REQUEST", leave.getId());

            return toResponse(leaveRepo.save(leave));
        } else {
            // Still pending somewhere in the approval chain — cancel outright, nothing to reverse
            ...
            notificationService.createAndSend(
                    leave.getEmployee(), "Leave Cancelled",
    ...
    /**
     * HR confirms or denies a cancellation request for an already-approved leave.
     * Only called when status == CANCELLATION_PENDING.
     */
    @Transactional
    public LeaveDTOs.Response cancelAction(Long leaveId, Long hrId, LeaveDTOs.CancelActionRequest req) {
        LeaveRequest leave = findById(leaveId);

        if (leave.getStatus() != LeaveStatus.CANCELLATION_PENDING) {
            throw new IllegalStateException("Leave is not awaiting cancellation confirmation");
        }
        ...
        if (Boolean.TRUE.equals(req.getApprove())) {
            leave.setStatus(LeaveStatus.CANCELLED);
            leaveBalanceService.restoreBalance(leave.getEmployee(), leave.getLeaveType(), leave.getTotalDays());

            notificationService.createAndSend(
                    leave.getEmployee(),
                    "Cancellation Confirmed",
                    "HR confirmed your cancellation. " + leave.getTotalDays() + " day(s) restored to your balance.",
                    NotificationType.LEAVE_CANCELLED, "LEAVE_REQUEST", leave.getId());
        } else {
            // Denied — leave reverts back to its approved state
            leave.setStatus(LeaveStatus.APPROVED);

            notificationService.createAndSend(
                    leave.getEmployee(),
                    "Cancellation Denied",
                    "HR denied your cancellation request. Reason: " + req.getRemarks(),
                    NotificationType.GENERAL, "LEAVE_REQUEST", leave.getId());
        }
    ...
    private LeaveDTOs.Response toResponse(LeaveRequest l) {
        ...
        if (l.getManager() != null) {
            r.setManagerName(l.getManager().getFirstName() + " " + l.getManager().getLastName());
        }
        r.setManagerRemarks(l.getManagerRemarks());
        r.setManagerActionAt(l.getManagerActionAt());
        if (l.getApprovedBy() != null) {
            r.setApprovedByName(l.getApprovedBy().getFirstName() + " " + l.getApprovedBy().getLastName());
        }
